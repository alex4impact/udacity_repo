...
def test_class_names(data):
    ...
    # Implement a test that checks the "genre" column to make sure covers only known classes
    assert (
        data["genre"].isin(known_classes).all()
    ), "Genre names are not within the know classes"
...
def test_column_ranges(data):
    ...
    for col_name, (minimum, maximum) in ranges.items():
        # Check that the values in the column col_name are within the expected range
        assert data[col_name].dropna().between(minimum, maximum).all(), (
            f"Values of the column {col_name} are not within the {minimum} and {maximum} range"
